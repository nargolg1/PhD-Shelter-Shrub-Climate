...
animal_species_compiled_spring <-animal_species_compiled_spring%>% filter(total %in% (5:1000))#you have to filter 5 and above if you want it not to show as empty rows in the bar graph


#FIGURE 5 WINTER

ggplot(animal_species_compiled_winter, aes(factor(scientific_name), total, fill = microsite)) +
...
ggplot(animal_species_compiled_spring, aes(factor(scientific_name), total, fill = microsite)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  theme_classic() +
  xlab("Species") +
  ...
  labs(fill = "Microsite") + theme_classic()+ theme(panel.border = element_rect(color = "black",
                                    fill = NA,
                                    size = 1)) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + facet_grid(~year, scales = "free")
```
