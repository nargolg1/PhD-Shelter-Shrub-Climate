...
mod1.3 <- rma(yi = yi, sei = vi, method = "ML", mods = ~days, test = "knha", control=list(stepadj=0.5), data = data_e1)
summary(mod1.3)
m1.3 <- tidy(mod1.3)
m1.3
plot(mod1.3)
#useless because days is now part of the effect size calculations.

#x = richness
#y = number of cams
#y = days
mod2.1 <- rma(yi = yi, sei = vi, method = "ML", test = "knha", control=list(stepadj=0.5), data = data_e3)
summary(mod2.1)
m2.1 <- tidy(mod2.1)
m2.1


mod2.2 <- rma(yi = yi, sei = vi, method = "ML", mods = ~ecosystem -1, test = "knha", control=list(stepadj=0.5), data = data_e3)
summary(mod2.2)
m2.2 <- tidy(mod2.2)
m2.2
plot(mod2.2)

mod2.3 <- rma(yi = yi, sei = vi, method = "ML", mods = ~days, test = "knha", control=list(stepadj=0.5), data = data_e3)
summary(mod2.3)
m2.3 <- tidy(mod2.3)
m2.3
...
## Viz  
Need to weight points but sampling effort - n_cams/number of days only for capture rate.
...
ggplot(data_e3, aes(ecosystem, yi)) +
  geom_boxplot() +
...
ggplot(data_e1, aes(days, yi)) +
  ...
  labs(x = "Total Days Sampled per Study", y = "Abundance capture rates")+ theme_classic()+ labs(colour = "Ecosystem")

ggplot(data_e3, aes(days, yi, weight = vi)) +
  geom_point(aes(color = ecosystem)) + 
  stat_smooth(method = "lm", formula = y ~ x + I(x^2), size = 2, color = "black", size = 2, se = FALSE) +
...
3. Increasing n_cams returns the highest rate of captures and of diversity in animals detected in deserts and grasslands.   
4. Increasing the number of days does not increase the capacity of the number of cams to detect more animals (either in captures or diversity).  
